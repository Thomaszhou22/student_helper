...
Deno.serve(async () => {
  try {
    ...
    let sentCount = 0;
    const debug: string[] = [];
    debug.push(`settings: ${settings.length}, tasks: ${tasks.length}`);
    debug.push(`now=${now.toISOString()}, in24h=${in24h.toISOString()}`);

    for (const setting of settings) {
      const userTasks = (tasks as DeadlineTask[]).filter((t) => t.user_id === setting.user_id);
      debug.push(`user ${setting.user_id}: ${userTasks.length} tasks`);
      if (userTasks.length === 0) continue;

      // Get user email via Supabase Admin API
      const supabaseUrl = Deno.env.get("SUPABASE_URL")!;
      const serviceKey = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!;
      const authRes = await fetch(`${supabaseUrl}/auth/v1/admin/users/${setting.user_id}`, {
        headers: { Authorization: `Bearer ${serviceKey}` },
      });
      if (!authRes.ok) {
        debug.push(`auth fetch failed: ${authRes.status} ${await authRes.text()}`);
        continue;
      }
      const authData = await authRes.json();
      const userEmail = authData.user?.email as string | undefined;
      if (!userEmail) {
        debug.push(`no email for user ${setting.user_id}`);
        continue;
      }
      debug.push(`email: ${userEmail}`);
      ...
      for (const task of userTasks) {
        ...
        debug.push(`task "${title}": dueAt=${dueAt.toISOString()}, category=${category}, status=${task.payload.status}`);

        // 24h check: task is due within the next 24 hours
        if (dueAt > now && dueAt <= in24h) {
          remindersToSend.push("24h");
          debug.push(`-> matches 24h window`);
        }
        ...
        for (const reminderType of remindersToSend) {
          ...
          if (eventError || !event) {
            debug.push(`insert event failed: ${eventError?.message}`);
            console.error(`Failed to create reminder event: ${eventError?.message}`);
            continue;
          }
          debug.push(`event created: id=${event.id}`);
    ...
    return new Response(
      JSON.stringify({ sent: sentCount, debug }),
